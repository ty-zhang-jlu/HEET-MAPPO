...
class EnvCore(object):
    ...
    def __init__(self):
        ...
        self.pin_min = 40
        self.yita = 0.7
        self.Jammer_position = [1000, 1000, 200]
        self.P_Jammer = 15  # Jammer发射功率约束
        self.J = 3  # Jammer天线数量
        self.Sigma_csi_Jammer = 0.02  # Jammer处CSI error
        self.dJ = 0.2  # Jammer天线间距

        self.agent_num = self.N + 1  # 设置智能体的个数
        self.obs_dim = self.K + 2  # 设置智能体的观测维度
        self.action_dim = [2, 2 * self.M * self.K]  # 设置智能体的动作维度
        self.done = False
    ...
    def initial(self):
        ...
        for i in range(self.agent_num):
            sub_obs = self.get_state(sum_power_in_0, h_k_list_0)  # 每个agent的观察空间一致
    ...
    def step(self, action_G, action_phase,action_tau, action_W):
        ...
        if self.done:
            ...
            return [sub_agent_obs, sub_agent_reward, sub_agent_done, sub_agent_info, j_agent_obs, j_agent_reward]


        sub_agent_reward, sub_agent_obs, j_agent_reward, j_agent_obs, sub_sum_rate = self.calculate_reward(action_G, action_phase,
                                                                                                             action_tau, action_W)
    ...
    def get_state(self, sum_power_in, h_k_list):
        ...
        # state_rate_min = np.array(rate_min, ndmin=1)
        # state_rate_list = np.array(rate_list).flatten()
        # state_sum_rate = np.array(sum_rate, ndmin=1)
        ...
        #state_action_G = action_G.reshape(-1)

        # state_list.append(state_tau)
        # state_list.append(state_rate_min)
        # state_list.append(state_rate_list)
        # state_list.append(state_sum_rate)
        state_list.append(state_sum_power_in)
        state_list.append(state_h_k_list)
        state_list.append(state_P_in)
        #state_list.append(state_action_G)
