import numpy as np
from global_matd3.utils.util import get_dim_from_space
from global_matd3.utils.segment_tree import SumSegmentTree, MinSegmentTree
...
class MlpReplayBuffer(object):
    def __init__(self, policy_info, policy_agents, buffer_size, use_same_share_obs, use_avail_acts,
        ...
        for p_id in self.policy_info.keys():
            string_keys = [key for key in self.policy_info[p_id].keys() if isinstance(key, str)]
            # print("String keys in policy_info for p_id {}: {}".format(p_id, string_keys))
            # print(self.policy_info[p_id]['obs_space'], self.policy_info[p_id]['share_obs_space'], self.policy_info[p_id]['act_space'],
            #       self.policy_info[p_id]['cent_obs_dim'], self.policy_info[p_id]['cent_act_dim'])
...
class MlpPolicyBuffer(object):
    ...
    def __init__(self, buffer_size, num_agents, obs_space, share_obs_space, act_space, use_same_share_obs,
        ...
        # print(obs_space, share_obs_space)
        self.obs = np.zeros(
            (self.buffer_size, self.num_agents, obs_space[0]), dtype=np.float32)

        if self.use_same_share_obs:
            self.share_obs = np.zeros((self.buffer_size, len(share_obs_space)), dtype=np.float32)
        else:
            self.share_obs = np.zeros((self.buffer_size, self.num_agents, len(share_obs_space)), dtype=np.float32)
    ...
    def insert(self, num_insert_steps, obs, share_obs, acts, rewards,
        ...
        # print("num_insert_steps:", num_insert_steps)
        # print('观察空间：', obs)
        ...
        if obs.ndim == 2:
            ...
            assert obs[0:1, :, :].shape[0] == num_insert_steps, ("different size!")
            self.obs[idx_range] = obs[0:1, :, :].copy()
        if next_obs.ndim == 4:
            next_obs = next_obs[0, 0, :, :][:, np.newaxis]
            next_obs = next_obs[0, 0, :].reshape(1, 1, -1)  # 使用 reshape 增加维度，-1 表示自动计算该维度

        self.share_obs[idx_range] = share_obs[0:1, :].copy()
        # self.acts[idx_range] = acts.copy()
        if acts.shape == (2,1):
            self.acts[idx_range] = acts.copy().T.reshape(1, 1, 2)
        elif acts.shape == (40,1):
            self.acts[idx_range] = acts.copy().T.reshape(1, 1, 40)
        else:
            self.acts[idx_range] = acts.copy()

        self.rewards[idx_range] = rewards.copy()
        self.next_obs[idx_range] = next_obs.copy()
        self.next_share_obs[idx_range] = next_obs.copy()
        dones = dones  # new
        dones_env = dones_env  # new
        valid_transition = valid_transition  # new
