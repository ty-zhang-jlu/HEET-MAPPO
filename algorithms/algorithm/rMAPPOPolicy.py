...
import torch
from global_mappo_8.algorithms.algorithm.r_actor_critic import R_Actor, R_Critic, DynamicCoeffNetwork
from global_mappo_8.utils.util import update_linear_schedule
import time
import numpy as np
...
class RMAPPOPolicy:
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        # 检查输入形状
        if len(obs.shape) == 3:  # [batch_size, num_agents, obs_dim]
            batch_size, num_agents, obs_dim = obs.shape

        actions, action_log_probs, rnn_states_actor, dist_entropy, attention_weights = self.actor(obs,
                                                                 rnn_states_actor,
                                                                 masks,
                                                                 available_actions,
                                                                 deterministic)
        actions_BS, action_log_probs_BS, rnn_states_actor_BS, dist_entropy_BS, attention_weights_BS = self.actor_BS(obs,
                                                                 rnn_states_actor,
                                                                 masks,
                                                                 available_actions,
                                                                 deterministic)
        actions_MBS, action_log_probs_MBS, rnn_states_actor_MBS, dist_entropy_MBS, attention_weights_MBS = self.actor_MBS(obs,
                                                                             rnn_states_actor,
                                                                             masks,
                                                                             available_actions,
                                                                             deterministic)
        ...
        return values, actions, action_log_probs, rnn_states_actor, actions_BS, action_log_probs_BS, rnn_states_actor_BS,\
               actions_MBS, action_log_probs_MBS, rnn_states_actor_MBS, rnn_states_critic, dist_entropy, dist_entropy_BS, dist_entropy_MBS,\
               attention_weights, attention_weights_BS, attention_weights_MBS
