...
import copy
import itertools
from global_matd3.utils.util import huber_loss, mse_loss, to_torch
from global_matd3.utils.popart import PopArt
from global_matd3.algorithms.base.trainer import Trainer
...
class MADDPG(Trainer):
    def __init__(self, args, num_agents, policies, policy_mapping_fn, device=None, actor_update_interval=1):
        ...
        self.policy_ids_sorted = sorted(self.policy_ids, key=lambda x: int(x.split('_')[1]))
        self.policy_agents = {policy_id: sorted(
            [agent_id for agent_id in range(self.num_agents) if self.policy_mapping_fn(agent_id) == policy_id]) for policy_id in
            self.policies.keys()}
        self.num_updates = {p_id : 0 for p_id in self.policy_ids_sorted}
    ...
    def get_update_info(self, update_policy_id, obs_batch, act_batch, nobs_batch, navail_act_batch):
        ...
        ind = 0
        for p_id in self.policy_ids_sorted:
            ...
            with torch.no_grad():
                cent_nact_id = []
                for batch in range(combined_nobs_batch.shape[0]):
                    pol_nact, _ = policy.get_cen_actions(combined_nobs_batch, combined_navail_act_batch, use_target=True)
                    ind_agent_nacts = pol_nact.cpu().numpy()
                ...
                # ind_agent_nacts = (pol_nact[0],)
                # ind_agent_nacts = pol_nact
            # cat to form the centralized next step actions
            #         print(ind_agent_nacts.shape)
                    cent_nact_id.append(ind_agent_nacts.T)
                    cent_nact_id_array = np.vstack(cent_nact_id)
            cent_nact.append(cent_nact_id_array)

            ind += num_pol_agents

        cent_act = list(itertools.chain.from_iterable(cent_act))
        # cent_nact_cpu = [tensor.cpu() for tensor in cent_nact]
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch):
        ...
        index = int(update_policy_id.replace('policy_', ''))
        cent_nact_id = cent_nact[index]

        update_policy = self.policies[update_policy_id]
        batch_size = cent_obs.shape[0]
        # critic update
        with torch.no_grad():
            next_step_Qs = update_policy.target_critic(cent_nobs, cent_nact_id)
        ...
        # detach the targets
        errors = [target_Qs.detach() - predicted_Q + 2 for predicted_Q in predicted_Qs]
        if self.use_per:
        ...
        if update_actor:
            ...
            # # agent_actor_batches = pol_acts
            agent_actor_batches_id = []
            for batch in range(pol_agents_obs_batch.shape[0]):
                pol_acts, _ = update_policy.get_cen_actions(pol_agents_obs_batch, pol_agents_avail_act_batch, use_gumbel=True)
                ind_agent_nacts = pol_acts.detach().cpu().numpy()
                agent_actor_batches_id.append(ind_agent_nacts.T)
                agent_actor_batches = np.vstack(agent_actor_batches_id)

            cent_act = list(map(lambda arr: to_torch(arr).to(**self.tpdv), cent_act))

            actor_cent_acts = copy.deepcopy(cent_act)
            for i in range(num_update_agents):
                actor_cent_acts[replace_ind_start + i] = agent_actor_batches

            # actor_cent_acts = torch.cat(actor_cent_acts[index], dim=-1).repeat(num_update_agents, 1)
            # # convert buffer acts to torch, formulate centralized buffer action and repeat as done above
            # buffer_cent_acts = torch.cat(cent_nact[index], dim=-1).repeat(num_update_agents, 1)
            buffer_cent_acts = torch.from_numpy(cent_nact[index]).to(device='cuda:0')
            actor_cent_acts_tensor = torch.from_numpy(actor_cent_acts[index]).to(device='cuda:0')
            ...
            # print(actor_cent_acts_tensor.shape, buffer_cent_acts.shape, mask.shape)
            actor_update_cent_acts = mask * actor_cent_acts_tensor + (1 - mask) * buffer_cent_acts
